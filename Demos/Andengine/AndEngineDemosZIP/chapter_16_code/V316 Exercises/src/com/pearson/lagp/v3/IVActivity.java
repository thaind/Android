...
public class IVActivity extends BaseGameActivity implements IOnSceneTouchListener, BKConstants {
	...
	private SharedPreferences audioOptions, scores, difficulty;
	...
	public Engine onLoadEngine() {
		...
		for (int i=0; i<NUM_LEVELS; i++){
			mLevelComplete[i] = difficulty.getBoolean("IV.LEVEL"+i, false);
		}
	...
	public Scene onLoadScene() {
		...
		try {
			for (int i=0; i<NUM_LEVELS; i++){
				if (!mLevelComplete[i]) {
					bkLoader.loadLevelFromAsset(getApplicationContext(), "iv"+i+".lvl");
					mCurrentLevel = i;
					break;
				}
			}
		...
		// Time display
		mTimeLeftTxt = new ChangeableText(0.2f*CAMERA_WIDTH, 10.0f, mFont32, "Secs: 0", "Secs: XXX".length());
		...
		mScene.registerUpdateHandler(new IUpdateHandler() {
			...
			public void onUpdate(final float pSecondsElapsed) {
				...
				mTimeLeftTxt.setText("Time: " + (int)mTimeLeft);
			}
		});
  ...
  public static String trimQuotes( String value )
  {
    ...
    return value;
     ...
     private void mGameOver(boolean pWin){
 		...
 		mScene.unregisterUpdateHandler(this.mPhysicsWorld);
    	 ...
    	 if (pWin){
    		 mPlayerWon = true;
    		 mLevelComplete[mCurrentLevel] = true;
    ...
    private Runnable mPlayThis = new Runnable() {
         public void run() {
     		Intent myIntent = new Intent(IVActivity.this, IVActivity.class);
     		IVActivity.this.startActivity(myIntent);
     		finish();
         }
    };
    ...
    private Runnable mPlayNext = new Runnable() {
         public void run() {
      		Intent myIntent = new Intent(IVActivity.this, Level1Activity.class);
      		IVActivity.this.startActivity(myIntent);
      		finish();
         }
    };
