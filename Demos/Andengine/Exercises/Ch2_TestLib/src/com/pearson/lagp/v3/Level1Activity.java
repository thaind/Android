...
public class Level1Activity extends BaseGameActivity {
	...
	private static final int MAX_VAM = 50;
	...
	private AnimatedSprite[] asprVamp = new AnimatedSprite[MAX_VAM];
	private int nVamp;
	Random gen;
	...
	public void onLoadResources() {
		...
		mScrumTextureRegion = TextureRegionFactory.createTiledFromAsset(
				mScrumTexture, this, "scrum_tiled.png", 0, 0, 8, 4);
	...
	public Scene onLoadScene() {
		...
		bullet.registerEntityModifier(new ParallelEntityModifier(
				new ParallelEntityModifier(new MoveYModifier(3, 0.0f,
						...
						new AlphaModifier(3, 0.0f, 1.0f), new ScaleModifier(3,
								0.1f, 1.0f)), new RotationModifier(3, 0, 360)));
		...
		cross.registerEntityModifier(new SequenceEntityModifier(
				new SequenceEntityModifier(new MoveYModifier(4, 0.0f,
						CAMERA_HEIGHT - 40.0f, EaseQuadOut.getInstance()),
		...
		hatchet.registerEntityModifier(new SequenceEntityModifier(
				new ParallelEntityModifier(new MoveYModifier(5, 0.0f,
						...
						new AlphaModifier(5, 0.0f, 1.0f), new ScaleModifier(5,
								0.5f, 1.0f)), new RotationModifier(5, 0, 360)));
	...
	private void displayNextVam() {
		mHandler.postDelayed(mStartVamp, 1000);
	}
	...
	private Runnable mStartVamp = new Runnable() {
		public void run() {
			...
			Arrays.fill(frameDurations, 100);
			asprVamp[i].animate(frameDurations, 0, 25, true);
			asprVamp[i].registerEntityModifier(new ParallelEntityModifier(
					new AlphaModifier(20.0f, 0.0f, 1.0f), new MoveModifier(
							20.0f, asprVamp[i].getX(), 30.0f, asprVamp[i]
