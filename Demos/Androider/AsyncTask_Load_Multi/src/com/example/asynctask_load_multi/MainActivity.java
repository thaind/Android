...
public class MainActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			new MyNetworkTask(5, targetImage, loadingProgressBar).execute(
					onLineURL0, onLineURL1, onLineURL2, onLineURL3, onLineURL4);
	...
	private class MyNetworkTask extends AsyncTask<URL, Integer, Void> {
		...
		protected Void doInBackground(URL... urls) {
			...
			if (urls.length > 0) {
				for (int i = 0; i < urls.length; i++) {
					...
					try {
						tBM[i] = BitmapFactory.decodeStream(networkUrl
		...
		protected void onPostExecute(Void result) {
			Toast.makeText(getBaseContext(), "Finished", Toast.LENGTH_LONG)
					.show();
		}
