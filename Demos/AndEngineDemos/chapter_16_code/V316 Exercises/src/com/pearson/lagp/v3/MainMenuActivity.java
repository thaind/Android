...
public class MainMenuActivity extends BaseGameActivity implements IOnMenuItemClickListener {
	...
	public void onResumeGame() {
		...
		audioOptions = getSharedPreferences("audio", MODE_PRIVATE);		
	   	if ((audioOptions.getBoolean("musicOn", false)) && ( StartActivity.mMusic != null )
	   			&& (StartActivity.mMusic.isPlaying())) StartActivity.mMusic.resume();
		if (mMainScene != null) mMainScene.registerEntityModifier(new ScaleAtModifier(0.5f, 0.0f, 1.0f, CAMERA_WIDTH/2, CAMERA_HEIGHT/2));
		if (mStaticMenuScene != null) mStaticMenuScene.registerEntityModifier(new ScaleAtModifier(0.5f, 0.0f, 1.0f, CAMERA_WIDTH/2, CAMERA_HEIGHT/2));
	}
    ...
    private void mShowHelp() {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(this)
                ...
                .setCancelable(false)
                .setMessage(message)
                .setPositiveButton(R.string.help_dialog_play, new DialogInterface.OnClickListener() {
        ...
        .setNegativeButton(R.string.help_dialog_cancel, new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int whichButton) {
            	dialog.dismiss();
            }
        });
    ...
    private Runnable mLaunchLevel1Task = new Runnable() {
     ...
     private Runnable mLaunchOptionsTask = new Runnable() {
      ...
      private Runnable mLaunchScoresTask = new Runnable() {
          public void run() {
      		Intent myIntent = new Intent(MainMenuActivity.this, ScoresActivity.class);
      		MainMenuActivity.this.startActivity(myIntent);
          }
       };
